...
class Maze {
    constructor(width,height) {
        ...
        this.loops = {percent:false, value:100};
    ...
    buildWall(i,d) { //i= index, d = direction (0,1,2,3)(up right down left)
        ...
        if (this.cells[i].walls[d] == 0) {
            ...
            this.cells[this.neighbor(i,d)].walls[(d+2)%4]=1; //make sure this works before deleting stuff below.
            /*switch (d) {
                case 0: this.cells[i-this.width].walls[2]=0; break;
                case 1: this.cells[i+1].walls[3]=0; break;
                case 2: this.cells[i+this.width].walls[0]=0; break;
                case 3: this.cells[i-1].walls[1]=0; break;
                default: break;
            }*/
        }
        return _ret;
    ...
    carveMaze() {//Method to make the maze.
        ...
        for (let i=0; i<this.loops.value; i++) {
    ...
    draw(context) {
        ...
        if (this.redraw.gold) { //this.redraw.gold
            ...
            context.resetTransform();
            context.drawImage(goldImage,0,0);
        }
        context.restore();
    } // draw(context)
    ...
    pathToGold(x,y) { // Returns direction 0-3 of shortest path to gold.
        ...
        this.cells[localIndex].walls.forEach((e,i) => {
            if (e==0) {
                ...
                if (distTest<test) {chosenExit = i; test = distTest;}
            }
        })
    ...
    removeWall(i,d) { //i= index, d = direction (0,1,2,3)(up right down left)
        ...
        if (this.cells[i].walls[d] == 1) {
            ...
            this.cells[this.neighbor(i,d)].walls[(d+2)%4]=0; //make sure this works before deleting stuff below.
            /*switch (d) {
                case 0: this.cells[i-this.width].walls[2]=0; break;
                case 1: this.cells[i+1].walls[3]=0; break;
                case 2: this.cells[i+this.width].walls[0]=0; break;
